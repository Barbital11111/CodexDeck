...
当前版本重点覆盖账号/API 管理、额度可视化和通知规则基础能力。远程控制入口已经预留，后续会接入手机端或远程管理流程。
...
The current release focuses on account/API management, quota visibility, and the first notification workflow foundation. Remote control is reserved for a future milestone.
