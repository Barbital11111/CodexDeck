...
mod tests {
    ...
    fn writes_provider_auth_file_as_flat_openai_credentials() {
        ...
        assert_eq!(parsed.get("access").and_then(Value::as_str), Some("atk"));
        assert_eq!(parsed.get("refresh").and_then(Value::as_str), Some("rtk"));
        assert_eq!(
    ...
    fn writes_shared_auth_store_under_openai_key() {
        ...
        assert_eq!(openai.get("access").and_then(Value::as_str), Some("atk"));
        assert_eq!(openai.get("refresh").and_then(Value::as_str), Some("rtk"));
        assert_eq!(
