...
function Get-WorkspaceRoot([string]$Root) {
  ...
  while (-not [string]::IsNullOrWhiteSpace($current)) {
    if ((Split-Path -Leaf $current) -eq "codexdeck-worktrees") {
...
CodexDeck is a desktop workspace for Codex account management, API management, and notification routes.
...
Set-TextReplace -Path (Join-Path $release "package-lock.json") -Map @{
  '"name": "codexdeck"' = '"name": "codexdeck"'
  'https://github.com/Barbital11111/CodexDeck' = 'https://github.com/Barbital11111/CodexDeck'
}

Set-TextReplace -Path (Join-Path $release "src-tauri\Cargo.toml") -Map @{
  'repository = "https://github.com/Barbital11111/CodexDeck"' = 'repository = "https://github.com/Barbital11111/CodexDeck"'
}

Set-TextReplace -Path (Join-Path $release "src\constants\externalLinks.ts") -Map @{
  'https://github.com/Barbital11111/CodexDeck' = 'https://github.com/Barbital11111/CodexDeck'
  'github.com/Barbital11111/CodexDeck' = 'github.com/Barbital11111/CodexDeck'
}
