...
export function displayAccountLabelFallback(
  ...
  return fallbackIsRelay ? normalized : "账号已隐藏";
}
...
export function displayThreadName(
  threadName: string | null | undefined,
  fallback: string,
  hideAccountDetails: boolean,
) {
  const normalized = normalizedText(threadName);
  if (hideAccountDetails) {
    return "线程已隐藏";
  }
...
export function displayProxyKeyLabel(
...
export function displaySecretValue(value: string | null | undefined, hideAccountDetails: boolean) {
  const normalized = normalizedText(value);
  if (!normalized) {
    return EMPTY_VALUE;
  }

  return hideAccountDetails ? maskText(normalized, 4, 4) : normalized;
}

export function displayUrlValue(value: string | null | undefined, hideAccountDetails: boolean) {
  const normalized = normalizedText(value);
  if (!normalized) {
    return EMPTY_VALUE;
  }

  return hideAccountDetails ? "地址已隐藏" : normalized;
}
